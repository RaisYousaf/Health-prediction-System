<?php
	...
	class Illness_Symptom {
		protected static $table_name = "illness_symptoms"; 
		protected static $db_fields = array("i_s_id", "s_id", "illness_id");
		...
		public function make($illness_id, $name){
			if(!empty($illness_id) && !empty($name)){
				$symptom = new Symptom();
				$symptom->illness_id = $illness_id;
				$symptom->name = $name;
		...
		public static function find_all_by_id($id = 0){
			if($id != 0){
				...
				$query = "SELECT * FROM illness_symptoms ".
				//"INNER JOIN illness USING(illness_id) ".
				" WHERE s_id = '".$database->escape_value($id)."'  ORDER BY illness_id ASC";
